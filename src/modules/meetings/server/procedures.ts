import { db } from "@/db";
import { meetings, agents, meetingMessages } from "@/db/schema";
...
export const meetingsRouter = createTRPCRouter({
    ...
    update: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            ...
            const call = streamVideo.video.call("default", updatedMeeting.id);
            await call.create({
                data: {
                    ...
                    custom: {
                        ...
                        agentId: updatedMeeting.agentId, // Add agentId for webhook access
                    },
                    settings_override: {
                        transcription: {
                            language: "en",
    ...
    getOne: protectedProcedure
     ...
     create: protectedProcedure.input(meetingsInsertSchema).mutation(async ({ input, ctx }) => {
         ...
         // Create Stream call for the meeting
         ...
         await call.create({
             data: {
                 ...
                 custom: {
                     ...
                     agentId: createdMeeting.agentId, // Add agentId for webhook access
                 },
                 settings_override: {
                     transcription: {
                         language: "en",
         ...
         // Get and upsert agent user in Stream
         const [existingAgent] = await db
             .select()
             .from(agents)
         ...
         console.log(`[Create Meeting] Stream call created for meeting ${createdMeeting.id}`);
