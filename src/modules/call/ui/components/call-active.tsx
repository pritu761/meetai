...
export const CallActive = ({ onLeave, meetingName, agentId, instructions, model }: Props) => {
...
const sendMessage = async (text: string) => {
    ...
    if (agentId) {
      try {
        await fetch(`/api/trpc/meetings.saveMessage`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            json: {
              meetingId: agentId, // Using agentId as meetingId for now - this should be fixed
              content: text,
              isAgent: false,
            }
          })
             ...
             {isAgentTyping && (
               <div className="flex flex-col items-start">
                 <div className="flex items-center gap-1.5 mb-1">
                   ...
                   <span className="text-xs text-white/50 font-medium">AI Agent</span>
                   <span className="text-xs text-white/30">{new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                 </div>
                 <div className="max-w-[85%] px-3 py-2 rounded-2xl text-sm bg-blue-500/20 text-blue-100 rounded-bl-md animate-pulse">
                   Thinking...
                 </div>
               </div>
             )}
